...
class ShallowModel:
    ...
    def _output_cell_line(self, input_cell_line):
        conv_cell_line_1 = Conv1D(filters=64, kernel_size=700, strides=5, activation='tanh')(input_cell_line)
        maxpool_cell_line_1 = MaxPooling1D(pool_size=5)(conv_cell_line_1)
        flatten_cell_line = Flatten()(maxpool_cell_line_1)
    ...
    def _output_drug(self, input_drug):
        conv_drug_1 = Conv1D(filters=64, kernel_size=200, strides=3, activation='tanh')(input_drug)
        maxpool_drug_1 = MaxPooling1D(pool_size=5)(conv_drug_1)
        flatten_drug = Flatten()(maxpool_drug_1)
    ...
    def _concatenate(self, outputs_cell_line, outputs_drug):
        ...
        dropout_2 = Dropout(0.1)(maxpool_2)
        flatten = Flatten()(dropout_2)
        dense_2 = Dense(40)(flatten)
